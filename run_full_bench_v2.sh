...
# Best pressure config: GAMG 1 V-cycle with GPU-parallel twoStageGaussSeidel smoother
...
echo "=== Full benchmark v2: GAMG-1cyc-twoStageGS + fusedMomentumSweeps=2, $NITERS iters, duct 885K ==="
echo ""
printf "%-20s %7s  %7s  %7s  %7s  %7s  %7s  %6s\n" "MODEL" "TOTAL" "U_asm" "U_slv" "p_set" "p_slv" "turb" "cont"
echo "-------------------- -------  -------  -------  -------  -------  -------  ------"

for entry in "${MODELS[@]}"; do
    IFS='|' read -r model_name turb_props <<< "$entry"
    dir="$BASE/bench_v2_${model_name}"
    rm -rf "$dir"
    cp -r "$TEMPLATE" "$dir"
...
SIMPLE { nNonOrthogonalCorrectors 0; consistent yes; pRefCell 0; pRefValue 0; fusedMomentumSweeps 2; }
...
EOF2
    ...
    if [ "$iters" -ge "$NITERS" ]; then
        ...
        printf "%-20s %7.1f  %7.1f  %7.1f  %7.1f  %7.1f  %7.1f  %s\n" \
